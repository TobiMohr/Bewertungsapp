              ...
              >
                ...
                <div v-if="uc.criterion.type === 'countable'" class="text-gray-700 font-bold text-lg">
                  ...
                  <div class="text-sm text-gray-500">
                    ⌀ Avg: {{
                      (criterionAverages.find(a => a.criterion_id === uc.criterion.id)?.average_value ?? 0)
                    }}
                  </div>
                </div>
  ...
  import { getCriterionAverages } from "@/live-sessions/api/sessions";
    ...
    methods: {
      ...
      findSessionById(sessions, id) {
        for (const session of sessions) {
          ...
          if (session.children?.length) {
            const found = this.findSessionById(session.children, id);
      ...
      async fetchUserData(userId) {
        try {
          ...
          const newSessions = evalRes.data;

          const previousSelectedId = this.selectedItem?.id;
          const matchedItem = this.findSessionById(newSessions, previousSelectedId);

          this.sessions = newSessions;
          this.selectedItem = matchedItem || newSessions[0] || null;
          if (this.selectedItem) {
            await Promise.all([
              this.fetchUserRole(userId, this.selectedItem.id),
              this.fetchCriterionAverages(this.selectedItem.id),
            ]);
          }
        } catch (err) {
          this.user = null;
      ...
      async fetchCriterionAverages(sessionId) {
        try {
          const res = await getCriterionAverages(sessionId);
          this.criterionAverages = res.data; // ✅ use data directly
        } catch (err) {
      ...
      handleSelect(item) {
        ...
        this.fetchCriterionAverages(item?.id);
      },
    },
    async mounted() {
      ...
      await this.fetchCriterionAverages(this.selectedItem?.id);
    },
  };
  </script>
